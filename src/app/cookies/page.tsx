...
export default function CookiesPage() {
  return (
    <LegalPage title="Cookie Policy" lastUpdated="April 26, 2026">
      <Section title="1. What this Policy covers">
        ...
        <p>
          <strong>This Policy applies to the chapter3five website</strong>{" "}
          (chapter3five.app). The chapter3five mobile applications (iOS,
          Android) do <em>not</em> set browser cookies, but they do store
          equivalent device-level data — see Section 5.
        </p>
      </Section>
      ...
      <Section title="5. Mobile applications — equivalent device storage">
        <p>
          Our mobile apps don&rsquo;t use cookies. Instead, they use the
          following device-level storage:
        </p>
        <ul>
          <li>
            <strong>Authentication tokens</strong> — kept in the
            device&rsquo;s secure storage so you stay signed in. Cleared
            when you sign out or uninstall the app.
          </li>
          <li>
            <strong>Cached questions and preferences</strong> — kept in
            local storage to speed up onboarding and offline reads.
          </li>
          <li>
            <strong>Push notification token</strong> — generated by Apple
            (APNs) or Google (FCM) via Expo and shared with us so we can
            deliver proactive messages. You can revoke this in your
            device&rsquo;s notification settings, or by signing out.
          </li>
        </ul>
        <p>
          We do <strong>not</strong> use any third-party SDK for mobile
