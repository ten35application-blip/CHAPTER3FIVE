...
export type FacePrompt = {
  ...
  /** Deterministic per-oracle: same id → same seed → same face on
      regeneration (barring model version drift). Equal to
      faceSeed(oracleId, 0); collision retries mutate via faceSeed(id, N). */
  seed: number;
};

/**
 * Deterministic seed for generation attempt N.
 *
 * Attempt 0 (the normal case): hash(oracleId) — same id, same seed, same
 * face on regeneration. Attempts 1–3 (avatar_hash collision retries):
 * hash(`${oracleId}:${N}`) — a deterministic mutation, so even the retry
 * ladder is reproducible per oracle. stableSeed() is the repo's standard
 * hash → positive 31-bit int (% 2^31 − 1), which Replicate accepts.
 */
export function faceSeed(oracleId: string, attempt: number = 0): number {
  return attempt === 0
    ? stableSeed(oracleId)
    : stableSeed(`${oracleId}:${attempt}`);
}
