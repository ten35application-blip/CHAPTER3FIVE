...
export function buildConciergePricingBlock(): string {
  return [
    ...
    "- Can record their own legacy archive (answer the forty questions themselves and mint a code others can redeem)",
    ...
    "- Can record their own legacy archive, same as Basic",
    "- Cancel any time. No refunds mid-month.",
    "",
    "INHERIT CODES (redeeming an identity someone shared). No plan includes or requires this — it is per code, on ANY tier:",
    "- If the person who made the code has PASSED AWAY, redeeming it is FREE. Always. This is the memorial waiver — grief is never a paywall moment.",
    `- If the person who made the code is living (shared ahead of time, or as a gift), redeeming it is a ${INHERITED_SLOT_PRICE_LABEL} ONE-TIME purchase — once per code, never a monthly fee`,
    "",
    ...
    "",
    "NO TRIAL. Free is real -- someone can use you (Adrian) forever without paying. Basic is the first paid step (their own identities + the legacy archive); Pro is the bigger cast with more room every month.",
  ].join("\n");
}
